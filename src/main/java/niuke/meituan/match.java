package niuke.meituan;//0.000 0.133 0.210 0.292 0.670 0.270 0.953 0.353 0.328 0.128 0.873 0.082 0.771 0.300 0.405 0.455
...
public class match {
    public static void main(String[] args) {
        ...
        for(int i =1;i<4;i++){
            ...
            for (int j = 0; j <16 ; j++) {
                int l = j+2^i/2^i ;
                //进1/4 1要和dp3,4打 2要和3，4打 4要和1，2打 3要和1，2打
                //group=4,8,16
                int inergroup = i<<i;
                int group= 1<<(i+1);
                System.out.println("这是几个人的比赛"+group);
                System.out.println("这几个人中有几个人是同一组的"+inergroup);
                int start = j/group;
                System.out.println(start);
                for(;start<start+group;start++) {
                    //如果j =1/2 k应该=3,4
                    //如果j = 3,4 k应该=1,2
                    //如果 j = 1 k应该=5，6，7，8

                    if(start/inergroup == start/inergroup)continue;
                    dp[i][j] += dp[i - 1][j] * dp[i - 1][start] * p[j][start];
